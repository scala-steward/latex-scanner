...
object CmdScanner extends App with OptionSupport with FileSupport with RegexSupport {
  ...
  def scan(config: Config) = {
    val items = for {
      path        <- (Find allFilesEndingWith ".tex" in config.workDir).par if ! config.ignoredTexFiles.contains(path.getFileName.toString)
