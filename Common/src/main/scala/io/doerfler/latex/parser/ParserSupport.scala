...
trait ParserSupport extends GeneralParserSupport {
  ...
  def sectionWithContent[_: P] = P(
    section ~ contentAfterSection map Section.tupled
  )
  def section[_: P] = P(
    "\\" ~ level ~ "{" ~/ (AnyString ~? "\\status") ~ status.! ~ "{}" ~ "}"
  )
  ...
  def cmd[_: P]: P[CmdOrText] = P(
    "\\" ~ cmdName ~ "{" ~/ stuff ~ "}" map (Cmd.apply _).tupled
  )
  def cmdName[_: P]: P[String] = P(CharIn("a-zA-Z0-9").rep.!)
  def text[_: P]: P[CmdOrText] = P(
    (CharIn("a-zA-Z0-9 \n\r\t") | "\\&").rep(1).! map Text
  )
}
...
trait GeneralParserSupport {
  case object AnyString {
    def ~?[_: P](termination: String): P[String] = P(
      (!termination ~ AnyChar).rep.! ~ termination
    )
  }
  ...
  def verboser[T](res: Parsed[T]): Either[String, T] =
    res.fold(
      (l, i, e) =>
        Left(
          "\n" + e.input
            .asInstanceOf[IndexedParserInput]
            .data + "\n" + (" " * i) + "^" + "\n" + e.trace().longAggregateMsg
        ),
      (t, i) => Right(t)
    )
